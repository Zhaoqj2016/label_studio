...
# 设置默认语言为中文
LANGUAGE_CODE = 'zh-hans'  # 使用连字符格式，符合 Django 标准
...
try:
    ...
    # 替换 faker 库的默认实例
    Faker._DEFAULT_LOCALE = 'zh_CN'
    
    # 替换 Faker 类的 __init__ 方法
    original_init = Faker.__init__
    
    def patched_init(self, locale=None, providers=None, generator=None, includes=None, **config):
        if locale is None:
            locale = 'zh_CN'
        original_init(self, locale, providers, generator, includes, **config)
    
    Faker.__init__ = patched_init
except ImportError:
    pass
